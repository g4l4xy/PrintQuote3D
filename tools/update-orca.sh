#!/bin/bash
set -euo pipefail
# The pinned upstream reference used for the bundled, deliberately small selection.
...
swift run --package-path "$PROJECT_DIR" OrcaProfileImporter --profiles "$TEMP_DIR/resources/profiles" --commit "$ACTUAL_SHA" --selection "$PROJECT_DIR/tools/orca-selection.json" --output "$TEMP_DIR/catalog.json"
